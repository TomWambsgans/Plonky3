//! AVX-512-optimized cubic extension field arithmetic for Goldilocks.
//!
//! See the AVX2 sibling for the rationale; the implementation strategy is
//! identical because the lane-wise SIMD ops (`+ - * square`) on
//! `PackedGoldilocksAVX512` are already efficient. We rearrange the
//! result-stage tree to minimise dep-chain depth and route every diagonal
//! `a_i^2` term through `square()` rather than `mul()`.
