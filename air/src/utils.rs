...
pub fn add3<AB: AirBuilder>(
    ...
    let acc = acc_16 + acc_32.mul_2exp_u64(16);

    builder.assert_zeros([
        acc * (acc + AB::Expr::from_prime_subfield(two_32))
            * (acc + AB::Expr::from_prime_subfield(two_32.double())),
        acc_16
            * (acc_16 + AB::Expr::from_prime_subfield(two_16))
...
pub fn add2<AB: AirBuilder>(
    ...
    builder.assert_zeros([
        acc * (acc + AB::Expr::from_prime_subfield(two_32)),
        acc_16 * (acc_16 + AB::Expr::from_prime_subfield(two_16)),
    ]);
}
