...
use super::{FullRound, PartialRound, RoundConstants24, SBox};

// TODO: Take generic iterable
#[instrument(name = "generate Poseidon2 trace", skip_all)]
pub fn generate_trace_rows_24<
    F: PrimeField,
...
>(
    ...
    generate_full_round::<_, LinearLayers, WIDTH, SBOX_DEGREE, SBOX_REGISTERS>(
        &mut state,
        &mut perm.last_full_round_1,
        &constants.ending_full_round_constants[0],
        &constants.ending_full_round_constants[1],
    );
    last_full_round::<_, LinearLayers, WIDTH, SBOX_DEGREE, SBOX_REGISTERS>(
        &mut state,
        &mut perm.last_full_round_2,
        &constants.ending_full_round_constants[2],
        &constants.ending_full_round_constants[3],
    );
}
...
>(
    state: &mut [F; WIDTH],
    full_round: &mut LastFullRound<MaybeUninit<F>, WIDTH, SBOX_DEGREE, SBOX_REGISTERS>,
    round_constants_1: &[F; WIDTH],
    round_constants_2: &[F; WIDTH],
) {
    // Combine addition of round constants and S-box application in a single loop
