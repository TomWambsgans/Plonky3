...
pub(super) fn binomial_square<F: Field, R: Algebra<F>, const D: usize>(
    ...
    match D {
        2 => {
            let a1_w = a[1] * w;
            res[0] = R::dot_product(a[..].try_into().unwrap(), &[a[0], a1_w]);
    ...
    R2: Algebra<F>,
{
    ...
    res[0] = R::dot_product(&[a[0], w_coeff_0], b_r_rev[3..].try_into().unwrap());
    ...
    R: Algebra<F>,
{
    ...
    res[3] = R::dot_product(&[two_a0, two_a2], &[a[3], a[1]]);
}
    ...
    R2: Algebra<F>,
{
    ...
    res[0] = R::dot_product(&[a[0], w_coeff_0], b_r_rev[4..].try_into().unwrap());
    ...
    R: Algebra<F>,
{
    ...
    let mut scratch: [R; D_PLUS_1_MIN_N] = array::from_fn(|i| a[i]);
